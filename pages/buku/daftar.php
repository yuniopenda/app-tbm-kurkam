...
if (isset($_SESSION['role']) && $_SESSION['role'] === 'anggota') {
    header("Location: /app-tbm-kurkam/pages/user/katalog.php");
    exit;
}
...
if ($filter_usia)
    $where .= " AND (kategori_usia = '$filter_usia' OR kategori_usia = 'semua')";
if ($filter_jenis)
    $where .= " AND jenis_buku = '$filter_jenis'";
...
<head>
    <meta charset="UTF-8">
    <meta nassme="viewport" content="width=device-width, initial-scale=1.0">
...
<body class="bg-slate-100 font-sans flex min-h-screen">
    ...
    <main class="flex-grow lg:ml-64 p-4 lg:p-8 pt-16 lg:pt-8">
        ...
        <div class="overflow-x-auto pills-scroll mb-4">
            <div class="flex items-center gap-2 pb-1" style="width: max-content;">
                ...
                <?php endforeach; ?>
                <span
                    class="text-xs text-slate-400 font-black uppercase tracking-widest whitespace-nowrap ml-2">Jenis:</span>
                ...
                <a href="?usia=<?= $filter_usia ?>&jenis=fisik"
                    class="whitespace-nowrap px-4 py-1.5 rounded-full text-xs font-bold transition <?= $filter_jenis == 'fisik' ? 'bg-indigo-600 text-white shadow-sm' : 'bg-white text-slate-500 border border-slate-200 hover:bg-slate-50' ?>">📖
                    Fisik</a>
                <a href="?usia=<?= $filter_usia ?>&jenis=digital"
                    class="whitespace-nowrap px-4 py-1.5 rounded-full text-xs font-bold transition <?= $filter_jenis == 'digital' ? 'bg-blue-600 text-white shadow-sm' : 'bg-white text-slate-500 border border-slate-200 hover:bg-slate-50' ?>">💻
                    Digital</a>
            </div>
        </div>
        ...
        <div class="flex gap-2 lg:gap-4 mb-5">
            ...
            <div class="flex items-center gap-2 bg-white px-3 py-2 rounded-xl border border-slate-200 shadow-sm">
                <span
                    class="text-[10px] font-black text-slate-400 uppercase tracking-widest hidden sm:inline">Baris:</span>
                <select onchange="location=this.value"
                    class="bg-transparent border-none text-sm font-bold text-indigo-600 outline-none cursor-pointer">
                    <option value="?limit=10&usia=<?= $filter_usia ?>&jenis=<?= $filter_jenis ?>"
                        <?= $limit == 10 ? 'selected' : '' ?>>10</option>
                    <option value="?limit=25&usia=<?= $filter_usia ?>&jenis=<?= $filter_jenis ?>"
                        <?= $limit == 25 ? 'selected' : '' ?>>25</option>
                    <option value="?limit=50&usia=<?= $filter_usia ?>&jenis=<?= $filter_jenis ?>"
                        <?= $limit == 50 ? 'selected' : '' ?>>50</option>
                </select>
            </div>
        </div>
    ...
    ══════════════════════════════════════ -->
        <div class="lg:hidden space-y-3">
            <?php if (!$found): ?>
                <div
                    class="bg-white rounded-3xl p-10 text-center text-slate-400 font-bold italic shadow-sm border border-slate-100">
            ...
            <?php foreach ($rows as $row):
                ...
                <div class="bg-white rounded-2xl shadow-sm border border-slate-100 p-4 flex gap-3 items-start">
                    ...
                    <!-- Cover -->
                    <div
                        class="w-12 h-16 flex-shrink-0 bg-slate-100 rounded-xl overflow-hidden border border-slate-200 flex items-center justify-center">
                        <?php if ($covOk): ?>
                    ...
                    <div class="flex-grow min-w-0">
                        <div class="flex items-start justify-between gap-2">
                            <div class="min-w-0 flex-1">
                                <span
                                    class="inline-block text-[10px] font-black text-indigo-500 bg-indigo-50 px-2 py-0.5 rounded mb-1"><?= htmlspecialchars($row['kode_buku']) ?></span>
                                <p class="font-black text-slate-800 text-sm leading-snug line-clamp-2">
                                    <?= htmlspecialchars($row['judul']) ?></p>
                                <p class="text-xs text-slate-400 mt-0.5 truncate">
                                    <?= htmlspecialchars($row['penulis'] ?? '-') ?> ·
                                    <?= htmlspecialchars($row['penerbit'] ?? '-') ?></p>
                            </div>
                            <!-- Tombol aksi -->
                        ...
                        <div class="flex flex-wrap gap-1.5 mt-2 items-center">
                            <?php if ($row['jenis_buku'] === 'digital'): ?>
                                <span class="px-2 py-0.5 bg-blue-100 text-blue-700 text-[10px] font-black rounded-full">💻
                                    Digital</span>
                            <?php else: ?>
                                <span class="px-2 py-0.5 bg-slate-100 text-slate-600 text-[10px] font-black rounded-full">📖
                                    Fisik</span>
                            <?php endif; ?>

                            <span
                                class="px-2 py-0.5 <?= $usia_badge[$kat] ?? 'bg-gray-100 text-gray-600' ?> text-[10px] font-black rounded-full">
                                <?= $usia_label[$kat] ?? $kat ?>
                            </span>

                            <?php if ($row['stok'] > 0): ?>
                                <span
                                    class="inline-flex items-center gap-1 px-2 py-0.5 bg-emerald-100 text-emerald-700 text-[10px] font-black rounded-full">
                                    <span class="w-1.5 h-1.5 bg-emerald-500 rounded-full"></span><?= $row['stok'] ?> Tersedia
                                </span>
                            <?php else: ?>
                                <span
                                    class="px-2 py-0.5 bg-red-100 text-red-600 text-[10px] font-black rounded-full italic">Habis</span>
                            <?php endif; ?>
                    ...
                    <tbody class="divide-y divide-slate-100">
                        ...
                        foreach ($rows as $row):
                            ...
                            <tr class="hover:bg-slate-50/60 transition">
                                ...
                                <td class="px-5 py-4">
                                    <div
                                        class="w-12 h-16 bg-slate-100 rounded-xl overflow-hidden border border-slate-200 flex items-center justify-center">
                                        <?php if ($covOk): ?>
                                            <img src="<?= $covUrl ?>" class="w-full h-full object-cover" alt="cover">
                                        <?php else: ?>
                                            <i class="fas fa-book text-slate-300 text-lg"></i>
                                ...
                                <td class="px-5 py-4">
                                    <div
                                        class="text-[10px] font-black text-indigo-500 bg-indigo-50 px-2 py-0.5 rounded w-fit mb-1">
                                        <?= htmlspecialchars($row['kode_buku']) ?></div>
                                ...
                                <td class="px-5 py-4">
                                    <div class="font-bold text-slate-700 text-sm">
                                        <?= htmlspecialchars($row['penulis'] ?? '-') ?></div>
                                    <div class="text-xs text-slate-400"><?= htmlspecialchars($row['penerbit'] ?? '-') ?>
                                    </div>
                                </td>
                                <td class="px-5 py-4">
                                    <?php if ($row['jenis_buku'] === 'digital'): ?>
                                        <span
                                            class="px-2.5 py-1 bg-blue-100 text-blue-700 text-[10px] font-black rounded-full">💻
                                            Digital</span>
                                    <?php else: ?>
                                        <span
                                            class="px-2.5 py-1 bg-slate-100 text-slate-600 text-[10px] font-black rounded-full">📖
                                            Fisik</span>
                                    <?php endif; ?>
                                    <div class="mt-1">
                                        <span
                                            class="px-2.5 py-1 <?= $usia_badge[$kat] ?? 'bg-gray-100 text-gray-600' ?> text-[10px] font-black rounded-full">
                                            <?= $usia_label[$kat] ?? $kat ?>
                                        </span>
                                    </div>
                                </td>
                                <td class="px-5 py-4 text-center">
                                    <?php if ($row['stok'] > 0): ?>
                                        <span
                                            class="inline-flex items-center gap-1.5 px-3 py-1 bg-emerald-100 text-emerald-700 text-xs font-black rounded-full">
                                            <span class="w-1.5 h-1.5 bg-emerald-500 rounded-full"></span><?= $row['stok'] ?>
                                            Tersedia
                                        </span>
                                    <?php else: ?>
                                        <span
                                            class="px-3 py-1 bg-red-100 text-red-600 text-xs font-black rounded-full italic">Habis</span>
                                    <?php endif; ?>
                                </td>
                                <td class="px-5 py-4 text-center">
                        ...
                        <?php if (!$found): ?>
                            <tr>
                                <td colspan="7" class="px-6 py-16 text-center text-slate-400 font-bold italic">Tidak ada
                                    buku ditemukan...</td>
                            </tr>
                        <?php endif; ?>
                    </tbody>
