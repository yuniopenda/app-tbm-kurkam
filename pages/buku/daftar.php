...
<main class="flex-grow lg:ml-64 p-4 lg:p-8 pt-16 lg:pt-8">
    <div class="flex flex-wrap justify-between items-start gap-3 mb-6">
        ...
        <div class="flex gap-2">
            <a href="template_excel.php" target="_blank"
               class="bg-emerald-600 hover:bg-emerald-700 text-white px-3 lg:px-5 py-2.5 lg:py-3 rounded-2xl font-bold shadow-lg transition flex items-center gap-2 text-sm">
                <i class="fas fa-file-excel"></i> <span class="hidden sm:inline">Template Excel</span>
            </a>
            <a href="tambah.php"
               class="bg-indigo-600 hover:bg-indigo-700 text-white px-4 lg:px-6 py-2.5 lg:py-3 rounded-2xl font-bold shadow-lg shadow-indigo-100 transition flex items-center gap-2 text-sm">
                <i class="fas fa-plus"></i> <span class="hidden sm:inline">Tambah Buku</span>
            </a>
        </div>
    </div>
    ...
    <div class="flex gap-2 mb-4 flex-wrap">
        ...
        <a href="?usia=<?= $val ?>&jenis=<?= $filter_jenis ?>"
           class="px-4 py-1.5 rounded-full text-xs font-bold transition <?= $active ? 'bg-indigo-600 text-white' : 'bg-white text-slate-500 border border-slate-200 hover:bg-slate-50' ?>">
            <?= $label ?>
        </a>
        <?php endforeach; ?>
        <span class="text-xs text-slate-400 font-black uppercase tracking-widest self-center ml-3">Jenis:</span>
        <a href="?usia=<?= $filter_usia ?>&jenis=" class="px-4 py-1.5 rounded-full text-xs font-bold transition <?= !$filter_jenis ? 'bg-indigo-600 text-white' : 'bg-white text-slate-500 border border-slate-200 hover:bg-slate-50' ?>">Semua</a>
        <a href="?usia=<?= $filter_usia ?>&jenis=fisik" class="px-4 py-1.5 rounded-full text-xs font-bold transition <?= $filter_jenis=='fisik' ? 'bg-indigo-600 text-white' : 'bg-white text-slate-500 border border-slate-200 hover:bg-slate-50' ?>">📖 Fisik</a>
        <a href="?usia=<?= $filter_usia ?>&jenis=digital" class="px-4 py-1.5 rounded-full text-xs font-bold transition <?= $filter_jenis=='digital' ? 'bg-blue-600 text-white' : 'bg-white text-slate-500 border border-slate-200 hover:bg-slate-50' ?>">💻 Digital</a>
    </div>

    <!-- Search + Limit -->
    <div class="flex gap-2 lg:gap-4 mb-6">
        <form method="POST" class="flex gap-2 flex-grow">
            <div class="relative flex-grow">
                <i class="fas fa-search absolute left-4 top-1/2 -translate-y-1/2 text-slate-400 text-sm"></i>
                <input type="text" name="keyword" value="<?= htmlspecialchars($keyword) ?>" placeholder="Cari judul, kode, penulis..."
            ...
            <button type="submit" name="cari" class="bg-indigo-600 text-white px-4 lg:px-6 py-3 rounded-xl font-bold hover:bg-indigo-700 transition">Cari</button>
        </form>
        <div class="flex items-center gap-2 bg-white px-3 lg:px-4 py-2 rounded-xl border border-slate-200 shadow-sm">
            ...
            <div class="flex gap-1">
                <?php for($i = 1; $i <= $jumlahHalaman; $i++): ?>
                    <a href="?halaman=<?= $i ?>&usia=<?= $filter_usia ?>&jenis=<?= $filter_jenis ?>"
                       class="w-8 h-8 flex items-center justify-center <?= $i==$halaman ? 'bg-indigo-600 text-white' : 'bg-white text-slate-500 border border-slate-200 hover:bg-slate-50' ?> rounded-lg text-xs font-bold transition"><?= $i ?></a>
                <?php endfor; ?>
            </div>
        </div>
    </div>
...
function hapusBuku(id) {
    Swal.fire({ title: 'Hapus Buku?', text: 'Data tidak bisa dikembalikan!', icon: 'warning',
